...
struct RootView: View {
    @StateObject private var rootNavigationManager: RootManagerClass = RootManagerClass()
    var body: some View {
        ZStack {
            LinearGradient(colors: [Color.init(hex: "#429FDB"), Color.init(hex: "#4249C2")], startPoint: .topLeading, endPoint: .bottomTrailing)
                .ignoresSafeArea()
            
            VStack {
                Group {
                    switch rootNavigationManager.viewPresentedNow {
                    case .loading:
                        ZagrujaemView()
                    case .menu:
                        MenuView()     
